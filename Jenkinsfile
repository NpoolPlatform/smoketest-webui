pipeline {
  ...
  stages {
    ...
    stage('Generate docker image for development') {
      ...
      steps {
        sh 'docker build -t $DOCKER_REGISTRY/entropypool/smoketest-webui-v1:latest .'
      }
    }
    ...
    stage('Release docker image for development') {
      ...
      steps {
        sh 'docker push $DOCKER_REGISTRY/entropypool/smoketest-webui-v1:latest'
        sh(returnStdout: true, script: '''
          images=`docker images | grep entropypool | grep smoketest-webui-v1 | grep none | awk '{ print $3 }'`
    ...
    stage('Release docker image for testing') {
      ...
      steps {
        sh(returnStdout: false, script: '''
          ...
          set +e
          docker images | grep smoketest-webui-v1 | grep $tag
          rc=$?
          set -e
          if [ 0 -eq $rc ]; then
            docker push $DOCKER_REGISTRY/entropypool/smoketest-webui-v1:$tag
          fi
        '''.stripIndent())
      }
    }

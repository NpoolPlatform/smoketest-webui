...
- Update语义：携带ID或EntID更新，或者携带ID和EntID更新，需要检查ID和EntID匹配以及资源有效性以及api权限，更新成功返回空，若找不到删除的对象，需要返回错误信息（由于现使用新的实现，当除了ID和EntID外，其他参数都未传的时候需要统一报错处理，如update nothing）
...
- 大后台操作的接口应该包含TargetAppID以及TargetUserID（如果需要获取指定APP或某用户信息）
